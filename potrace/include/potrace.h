...
struct backend_s {
	...
	int (*init_f)(std::vector<unsigned char> &fout, info_t *info);                 /* initialization function */
	int (*page_f)(void *pResult, potrace_path_t *plist, imginfo_t *imginfo, info_t *info);
	/* per-bitmap function */
	int (*term_f)(std::vector<unsigned char> &fout, info_t *info);                 /* finalization function */
